...
LABELS_PATH = "/Volumes/My Passport for Mac/data/dog-breeds/columns.csv"
...
def predict():
    ...
    for img_path in getImagesFilePathsFromFolder(UPLOAD_FOLDER):
        print(img_path)
        image_url = '127.0.0.1:8000/uploads/' + img_path.split("/")[-1]
        print(image_url)
        breed = predictInternal(get_x_test(img_path, 299, 299), loaded_model)[0]
        print(breed)
...
def upload_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            (breed, imageUrl) = predict()
            print(breed)
            print(imageUrl)
            return render_template('classifydogs.html', breed=breed ,imageUrl=imageUrl)
    else:
    	return render_template('classifydogs.html', review="" ,trolls=None)
