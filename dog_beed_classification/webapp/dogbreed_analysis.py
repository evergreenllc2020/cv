    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            if not (output is None):
                for imageUrl,breed in output.items():
                    print(imageUrl)
                    print(breed)
            return render_template('classifydogs.html', breed=breed ,output=output)
    else:
    	return render_template('classifydogs.html', review="" ,output=None)
